package com.carrentalproject.helper;
...
public class ExcelHelper {
    ...
    //Customer adında bir sayfa acicak excelde ve basliklarda olmasi gereken alanlari burada belirtiyoruz
    ...
    public static ByteArrayInputStream usersExcel(List<User> users)  { //datalarimizi byteArray olarak dondurecek
        try (Workbook workbook = new XSSFWorkbook();//
             ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            Sheet sheet = workbook.createSheet(SHEET_USER); //sheetleri olusturduk
            Row headerRow = sheet.createRow(0); // rowlari olusturduk 0 dan basladi ilk satir

            for (int column = 0; column < HEADERS_USER.length; column++) {
                Cell cell = headerRow.createCell(column); //cell leri olusturduk
                cell.setCellValue(HEADERS_USER[column]); // basliklarimizi hücrelere getirdik
            }

            int rowId = 1;
            for (User user: users) {
                Row row = sheet.createRow(rowId++);
               //her satirdaki hücrelere fieldlarimizin degerlerini atadik
            ...
            workbook.write(out); //objemize out verimizi yazdik
            return new ByteArrayInputStream(out.toByteArray()); //byteArray e cevirip dondurduk
